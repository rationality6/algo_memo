...
def vowel_2_index(string):
    vowel = 'aeiouAEIOU'
    return ''.join(str(i + 1) if t in vowel else t for i, t in enumerate(string))


Test.assert_equals(vowel_2_index('this is my string'), 'th3s 6s my str15ng')
Test.assert_equals(vowel_2_index('Codewars is the best site in the world'),
                   'C2d4w6rs 10s th15 b18st s23t25 27n th32 w35rld')
Test.assert_equals(vowel_2_index('Tomorrow is going to be raining'),
                   'T2m4rr7w 10s g1415ng t20 b23 r2627n29ng')
Test.assert_equals(vowel_2_index(''), '')
Test.assert_equals(vowel_2_index(
...
a = 36 * 35 * 34 * 33 * 32 * 31 * 30 * 29 * 28
import functools
# b = functools.reduce(lambda a,b:a*b,range(2,28))
c = functools.reduce(lambda a, b: a * b, range(2, 10))
print(a / c)

import itertools
c = itertools.combinations([*range(0, 36)], 9)
print(len([*c]))
...
print(compound_rate(11200, 0.15, 12))
